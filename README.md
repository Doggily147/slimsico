...
- `build_city.py` - the neon cyber city under a forcefield dome in the far corner of the plate: stone ring wall with turret pods every 45 degrees, an arched forcefield gate, a see-through dome on the wall top, avenues and a ring road, 116 lit towers, ground cars and flying cars keyed for the whole timeline. `render_city.py` renders the check views.
