...
- `build_lime.py` - Lime: a lime-green blob cloned from Yellow with round glasses and "LIME" written on his left forearm.
- `build_draft2_beat7.py` - draft 2, beat 7 (frames 1657-2680): Lime falls out of the sky onto the jetski, hangs off the side, climbs on and sits on it; the names exchange. Purple never speaks.
...
- `build_city.py` - the neon cyber city under a forcefield dome in the far corner of the plate: stone ring wall with turret pods every 45 degrees, an arched forcefield gate, a see-through dome on the wall top, avenues, a ring road, a plaza with a fountain and park, 116 towers with real windows and balconies (four landmarks, a hero spire), shops and lobbies with interiors, a monorail with trains, crosswalks, signage, blob citizens, cars with people in them and flyers, all keyed for the whole timeline. `render_city.py` renders the check views. A rebuild takes about 25 minutes.
- `renders/city_gate.png`, `city_aerial.png`, `city_street.png`, `city_plaza_up.png`, `city_from_plate.png` - the city from outside the gate, from the air, at street level, looking up from the plaza, and from the plate.
...
- `renders/draft2.mp4` - draft 2 so far: beats 1-7, 112 s, with music. `renders/draft2_clip_1081_1656.mp4` and `renders/draft2_clip_1657_2680.mp4` are beats 6 and 7 on their own.
- `renders/lime.png`, `renders/lime_hang.png` - Lime saying his name, and hanging off the side of the jetski.
...
the Text editor, then `build_draft2.py` for the current draft, and `build_crate.py`, `build_monster.py` then `build_draft2_beat4.py`, `build_draft2_beat5.py`, `build_draft2_beat6.py` and (after `build_lime.py`) `build_draft2_beat7.py` for beats 4 to 7 (`build_crate.py` clears the whole Props collection, so run `build_hovercraft.py` after it; beat 6 needs the jetski) (the beat scripts also run headless: `blender -b slimsico.blend --python build_draft2_beat5.py`). Finished drafts
